...
#include "canopen_402_driver/motor_amc.hpp"
using namespace ros2_canopen;

bool MotorAMC::setTarget(double val)
{
...
bool MotorAMC::isModeSupported(uint16_t mode)
{
  return mode != MotorBase::Homing && allocMode(mode);
}

bool MotorAMC::enterModeAndWait(uint16_t mode)
{
  bool okay = mode != MotorBase::Homing && switchMode(mode);
  return okay;
}

uint16_t MotorAMC::getMode()
{
  std::scoped_lock lock(mode_mutex_);
  return selected_mode_ ? selected_mode_->mode_id_ : (uint16_t)MotorBase::No_Mode;
}

bool MotorAMC::isModeSupportedByDevice(uint16_t mode)
{
...
void MotorAMC::registerMode(uint16_t id, const ModeSharedPtr & m)
{
  std::scoped_lock map_lock(map_mutex_);
  if (m && m->mode_id_ == id) modes_.insert(std::make_pair(id, m));
}

ModeSharedPtr MotorAMC::allocMode(uint16_t mode)
{
  ModeSharedPtr res;
...
bool MotorAMC::switchMode(uint16_t mode)
{
  ...
  if (!next_mode)
  {
    RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Mode is not supported.");
    return false;
  }

  if (!next_mode->start())
  {
    RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Could not  start mode.");
    return false;
  }
  ...
  {  // wait for switch
    ...
    {
      RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Mode switch timed out.");
...
bool MotorAMC::switchState(const State402::InternalState & target)
{
  std::chrono::steady_clock::time_point abstime =
  ...
  while (state != target_state_)
  {
    ...
    if (!success)
    {
      RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Could not set transition.");
      return false;
    }
    ...
    {
      RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Transition timed out.");
      if (enable_diagnostics_.load())
      {
...
bool MotorAMC::readState()
{
  ...
  // RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Mode %hhi",new_mode);

  if (selected_mode_ && selected_mode_->mode_id_ == new_mode)
  {
    if (!selected_mode_->read(sw))
    {
      RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Mode handler has error.");
    }
  }
  ...
  if (selected_mode_ && selected_mode_->mode_id_ != new_mode)
  {
    RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Mode does not match.");
  }
  if (sw & (1 << State402::SW_Internal_limit))
  {
    if (old_sw & (1 << State402::SW_Internal_limit))
    {
      RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Internal limit active");
    }
    else
    {
      RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Internal limit active");
...
void MotorAMC::handleRead() { readState(); }
void MotorAMC::handleWrite()
{
...
bool MotorAMC::handleInit()
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Init: Read State");
  if (!readState())
  {
  ...
  RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Init: Enable");
  ...
  RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Init: Switch to homing");
  if (!switchMode(MotorBase::Homing))
  {
    std::cout << "Could not enter homing mode" << std::endl;
    return false;
  }
  RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Init: Execute homing");
  if (!homing->executeHoming())
  {
    std::cout << "Homing failed" << std::endl;
    return false;
  }
  RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Init: Switch no mode");
...
bool MotorAMC::handleEnable()
{
  RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Enable: Read State");
  if (!readState())
  {
    std::cout << "Could not read motor state" << std::endl;
    return false;
  }
  RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Enable");
  if (!switchState(State402::Operation_Enable))
  {
    std::cout << "Could not enable motor" << std::endl;
    return false;
...
bool MotorAMC::handleDisable()
{
  RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Disable: Read State");
  if (!readState())
  {
    std::cout << "Could not read motor state" << std::endl;
    return false;
  }
  RCLCPP_INFO(rclcpp::get_logger("canopen_amc_driver"), "Disable");
