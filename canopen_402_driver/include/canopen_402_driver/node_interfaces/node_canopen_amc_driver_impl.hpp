...
#ifndef NODE_CANOPEN_AMC_DRIVER_IMPL_HPP_
#define NODE_CANOPEN_AMC_DRIVER_IMPL_HPP_

#include "canopen_402_driver/node_interfaces/node_canopen_amc_driver.hpp"
...
NodeCanopenAMCDriver<NODETYPE>::NodeCanopenAMCDriver(NODETYPE * node)
: ros2_canopen::node_interfaces::NodeCanopenProxyDriver<NODETYPE>(node)
{
}

template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::init(bool called_from_base)
{
  RCLCPP_ERROR(this->node_->get_logger(), "Not init implemented.");
}

template <>
void NodeCanopenAMCDriver<rclcpp::Node>::init(bool called_from_base)
{
  ...
  handle_set_mode_position_service = this->node_->create_service<std_srvs::srv::Trigger>(
    ...
    std::bind(
      &NodeCanopenAMCDriver<rclcpp::Node>::handle_set_mode_position, this, std::placeholders::_1,
      std::placeholders::_2));
  ...
  handle_set_mode_velocity_service = this->node_->create_service<std_srvs::srv::Trigger>(
    ...
    std::bind(
      &NodeCanopenAMCDriver<rclcpp::Node>::handle_set_mode_velocity, this, std::placeholders::_1,
      std::placeholders::_2));
  ...
  handle_set_mode_cyclic_velocity_service = this->node_->create_service<std_srvs::srv::Trigger>(
    ...
    std::bind(
      &NodeCanopenAMCDriver<rclcpp::Node>::handle_set_mode_cyclic_velocity, this,
  ...
  handle_set_mode_cyclic_position_service = this->node_->create_service<std_srvs::srv::Trigger>(
    ...
    std::bind(
      &NodeCanopenAMCDriver<rclcpp::Node>::handle_set_mode_cyclic_position, this,
      std::placeholders::_1, std::placeholders::_2));
  ...
  handle_set_mode_interpolated_position_service =
    this->node_->create_service<std_srvs::srv::Trigger>(
      ...
      std::bind(
        &NodeCanopenAMCDriver<rclcpp::Node>::handle_set_mode_interpolated_position, this,
        std::placeholders::_1, std::placeholders::_2));
  ...
  handle_set_mode_torque_service = this->node_->create_service<std_srvs::srv::Trigger>(
    ...
    std::bind(
      &NodeCanopenAMCDriver<rclcpp::Node>::handle_set_mode_torque, this, std::placeholders::_1,
      std::placeholders::_2));
  ...
  handle_set_mode_cyclic_torque_service = this->node_->create_service<std_srvs::srv::Trigger>(
    ...
    std::bind(
      &NodeCanopenAMCDriver<rclcpp::Node>::handle_set_mode_cyclic_torque, this,
      std::placeholders::_1, std::placeholders::_2));
  ...
  handle_set_target_service = this->node_->create_service<canopen_interfaces::srv::COTargetDouble>(
    ...
    std::bind(
      &NodeCanopenAMCDriver<rclcpp::Node>::handle_set_target, this, std::placeholders::_1,
      std::placeholders::_2));
}

template <>
void NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::init(bool called_from_base)
{
  ...
  handle_init_service = this->node_->create_service<std_srvs::srv::Trigger>(
    std::string(this->node_->get_name()).append("/init").c_str(),
    std::bind(
      &NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::handle_init, this,
      std::placeholders::_1, std::placeholders::_2));

  handle_enable_service = this->node_->create_service<std_srvs::srv::Trigger>(
    std::string(this->node_->get_name()).append("/enable").c_str(),
    std::bind(
      &NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::handle_enable, this,
      std::placeholders::_1, std::placeholders::_2));

  handle_disable_service = this->node_->create_service<std_srvs::srv::Trigger>(
    std::string(this->node_->get_name()).append("/disable").c_str(),
    std::bind(
      &NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::handle_disable, this,
      std::placeholders::_1, std::placeholders::_2));

  handle_halt_service = this->node_->create_service<std_srvs::srv::Trigger>(
    std::string(this->node_->get_name()).append("/halt").c_str(),
    std::bind(
      &NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::handle_halt, this,
      std::placeholders::_1, std::placeholders::_2));

  handle_recover_service = this->node_->create_service<std_srvs::srv::Trigger>(
    std::string(this->node_->get_name()).append("/recover").c_str(),
    std::bind(
      &NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::handle_recover, this,
      std::placeholders::_1, std::placeholders::_2));

  handle_set_mode_position_service = this->node_->create_service<std_srvs::srv::Trigger>(
    std::string(this->node_->get_name()).append("/position_mode").c_str(),
    std::bind(
      &NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::handle_set_mode_position, this,
      std::placeholders::_1, std::placeholders::_2));

  handle_set_mode_velocity_service = this->node_->create_service<std_srvs::srv::Trigger>(
    std::string(this->node_->get_name()).append("/velocity_mode").c_str(),
    std::bind(
      &NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::handle_set_mode_velocity, this,
      std::placeholders::_1, std::placeholders::_2));

  handle_set_mode_cyclic_velocity_service = this->node_->create_service<std_srvs::srv::Trigger>(
    std::string(this->node_->get_name()).append("/cyclic_velocity_mode").c_str(),
    std::bind(
      &NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::handle_set_mode_cyclic_velocity, this,
      std::placeholders::_1, std::placeholders::_2));

  handle_set_mode_cyclic_position_service = this->node_->create_service<std_srvs::srv::Trigger>(
    std::string(this->node_->get_name()).append("/cyclic_position_mode").c_str(),
    std::bind(
      &NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::handle_set_mode_cyclic_position, this,
      std::placeholders::_1, std::placeholders::_2));
  ...
  handle_set_mode_interpolated_position_service = this->node_->create_service<
    ...
    std::string(this->node_->get_name()).append("/interpolated_position_mode").c_str(),
    std::bind(
      &NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::handle_set_mode_interpolated_position,
      this, std::placeholders::_1, std::placeholders::_2));

  handle_set_mode_torque_service = this->node_->create_service<std_srvs::srv::Trigger>(
    std::string(this->node_->get_name()).append("/torque_mode").c_str(),
    std::bind(
      &NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::handle_set_mode_torque, this,
      std::placeholders::_1, std::placeholders::_2));

  handle_set_mode_cyclic_torque_service = this->node_->create_service<std_srvs::srv::Trigger>(
    std::string(this->node_->get_name()).append("/cyclic_torque_mode").c_str(),
    std::bind(
      &NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::handle_set_mode_cyclic_torque, this,
      std::placeholders::_1, std::placeholders::_2));

  handle_set_target_service = this->node_->create_service<canopen_interfaces::srv::COTargetDouble>(
    std::string(this->node_->get_name()).append("/target").c_str(),
    std::bind(
      &NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::handle_set_target, this,
      std::placeholders::_1, std::placeholders::_2));
}

template <>
void NodeCanopenAMCDriver<rclcpp_lifecycle::LifecycleNode>::configure(bool called_from_base)
{
...
template <>
void NodeCanopenAMCDriver<rclcpp::Node>::configure(bool called_from_base)
{
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::activate(bool called_from_base)
{
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::deactivate(bool called_from_base)
{
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::poll_timer_callback()
{
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::publish()
{
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::add_to_master()
{
  NodeCanopenProxyDriver<NODETYPE>::add_to_master();
  motor_ =
    std::make_shared<MotorAMC>(this->lely_driver_, switching_state_, homing_timeout_seconds_);
}

template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::handle_init(
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::handle_recover(
  const std_srvs::srv::Trigger::Request::SharedPtr request,
  std_srvs::srv::Trigger::Response::SharedPtr response)
{
  if (this->activated_.load())
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::handle_halt(
  const std_srvs::srv::Trigger::Request::SharedPtr request,
  std_srvs::srv::Trigger::Response::SharedPtr response)
{
  if (this->activated_.load())
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::handle_set_mode_position(
  const std_srvs::srv::Trigger::Request::SharedPtr request,
  std_srvs::srv::Trigger::Response::SharedPtr response)
{
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::handle_set_mode_velocity(
  const std_srvs::srv::Trigger::Request::SharedPtr request,
  std_srvs::srv::Trigger::Response::SharedPtr response)
{
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::handle_set_mode_cyclic_position(
  const std_srvs::srv::Trigger::Request::SharedPtr request,
  std_srvs::srv::Trigger::Response::SharedPtr response)
{
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::handle_set_mode_interpolated_position(
  const std_srvs::srv::Trigger::Request::SharedPtr request,
  std_srvs::srv::Trigger::Response::SharedPtr response)
{
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::handle_set_mode_cyclic_velocity(
  const std_srvs::srv::Trigger::Request::SharedPtr request,
  std_srvs::srv::Trigger::Response::SharedPtr response)
{
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::handle_set_mode_torque(
  const std_srvs::srv::Trigger::Request::SharedPtr request,
  std_srvs::srv::Trigger::Response::SharedPtr response)
{
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::handle_set_mode_cyclic_torque(
  const std_srvs::srv::Trigger::Request::SharedPtr request,
  std_srvs::srv::Trigger::Response::SharedPtr response)
{
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::handle_set_target(
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::handle_disable(
  const std_srvs::srv::Trigger::Request::SharedPtr request,
  std_srvs::srv::Trigger::Response::SharedPtr response)
{
  if (this->activated_.load())
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::handle_enable(
  const std_srvs::srv::Trigger::Request::SharedPtr request,
  std_srvs::srv::Trigger::Response::SharedPtr response)
{
  if (this->activated_.load())
...
template <class NODETYPE>
bool NodeCanopenAMCDriver<NODETYPE>::init_motor()
{
  if (this->activated_.load())
  {
    bool temp = motor_->handleInit();
...
template <class NODETYPE>
bool NodeCanopenAMCDriver<NODETYPE>::recover_motor()
{
  if (this->activated_.load())
  {
...
template <class NODETYPE>
bool NodeCanopenAMCDriver<NODETYPE>::halt_motor()
{
  if (this->activated_.load())
  {
...
template <class NODETYPE>
bool NodeCanopenAMCDriver<NODETYPE>::set_operation_mode(uint16_t mode)
{
  if (this->activated_.load())
  {
...
template <class NODETYPE>
bool NodeCanopenAMCDriver<NODETYPE>::set_target(double target)
{
  if (this->activated_.load())
  {
    auto mode = motor_->getMode();
...
template <class NODETYPE>
void NodeCanopenAMCDriver<NODETYPE>::diagnostic_callback(
